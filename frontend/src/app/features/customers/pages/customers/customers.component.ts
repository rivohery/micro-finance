...
export class CustomersComponent implements OnInit {
  ...
  customers = signal<CustomerMinResponse[]>([]);
  ...
  ngOnInit(): void {
    ...
    this.customerService.initFindAllEnableCustomersState();
    this.customerService.findAllEnableCustomers(
      this.currentSearch(),
      this.currentPage(),
      this.size
    );
  ...
  private handleFindAllEnableCustomersState(): void {
    const findAllEnableCustomersState =
      this.customerService.findAllEnableCustomersState$();
    if (
      findAllEnableCustomersState.status === 'OK' &&
      findAllEnableCustomersState.value
    ) {
      this.loading.set(false);
      this.customers.set(findAllEnableCustomersState.value.content || []);
