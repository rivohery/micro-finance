...
@Component({
  selector: 'app-customer-admin-detail',
  imports: [
    CommonModule,
    MatCardModule,
    MatIconModule,
  ...
  templateUrl: './customer-admin-detail.component.html',
  styleUrl: './customer-admin-detail.component.css',
})
export class CustomerAdminDetailComponent {
  ...
  constructor() {
    ...
    effect(() => {
      if (this.customerService.closeCustomerAccountState$().status === 'OK') {
        this.closingAccount.set(false);
        this.toastr.show(
          this.customerService.closeCustomerAccountState$().value?.message ||
            '',
          'SUCCESS'
        );
      ...
      if (
        this.customerService.closeCustomerAccountState$().status === 'ERROR'
      ) {
        this.closingAccount.set(false);
        this.errorMsg.set(
          this.customerService.closeCustomerAccountState$().error || ''
        );
      }
    });
  }
  ...
  ngOnInit(): void {
    this.customerService.initCloseCustomerAccountState();
    if (this.clientId()) {
      const clientId = this.clientId() || '';
      this.loading.set(true);
      this.customerService.findCustomerDetailsById(clientId);
    }
  }
  ...
  editClient(client: CustomerResponse): void {
    this.customerService.findCustomerDetailsById(client.id);
  ...
  closeAccount(client: CustomerResponse): void {
    ...
    this.customerService.closeCustomerAccount(client.id);
  }
}
